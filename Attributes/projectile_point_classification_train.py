...
def train_and_save_model(df,class_type, form, include_types, version, img_dir, model_dir, num_epochs = 20):
    ...
    df_merged = df_merged.sort_values(by=class_type).copy()
    ...
    for epoch in range(epochs):
        ...
        model_save_path = f"{model_dir}/class_{class_type}_{form_string}_epoch_{epoch + 1}_{version}.pth"
        log_file_path = f"{model_dir}/class_{class_type}_{form_string}_{version}.txt"
...
# usage
model_dir = "Attributes/data"
img_dir = "Attributes/images/ml"
data_path = f"{model_dir}/heurist_projectilePoints.csv"
...
print(class_counts)
train_and_save_model(df, class_type=class_type, form=form, include_types=include_types, version="v1", img_dir = img_dir, model_dir = model_dir, num_epochs = 20)
